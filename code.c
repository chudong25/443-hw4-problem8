...
int main()
{
    ...
    while (pair1_index != -1)
    {
        ...
        {
            double unmatched_bits = count_unmatched_bits(values[pair1_index], values[pair2_index]);
            printf("Pair 1 index= &ld ", values[pair1_index]);
            printf("Pair 2 index= &ld ", values[pair2_index]);
            printf("\nThe number of unmatched bits between value %d and value %d is: %f\n", pair1_index + 1, pair2_index + 1, unmatched_bits);
            printf("The distance is: %f\n", unmatched_bits / 64);
            printf("\nEnter the indices of the two values to compare(-1 to quit)\n");
            printf("\n0=Alice, 1=Bob, 2=Charlie, 3=U, 4=V, 5=W, 6=X, 7=Y:\n");
            printf("Pair 1 index: ");
            scanf("%d", &pair1_index);
            printf("Pair 2 index: ");
